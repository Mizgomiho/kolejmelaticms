...
$query=mysqli_query($con,"update users set fullName='$fname',contactNo='$contactno',address='$address',block='$block',roomnumber='$roomnumber' where userEmail='".$_SESSION['login']."'");
if($query)
{
...
<html lang="en">
  <head>
    ...
    <title>KOLEJ MELATI | User Change Password</title>
...
<div class="col-sm-4">
<select name="block" required="required" class="form-control">
<option value="<?php echo htmlentities($row['block']);?>"><?php echo htmlentities($st=$row['block']);?></option>
<?php $sql=mysqli_query($con,"select blockName from block ");
while ($rw=mysqli_fetch_array($sql)) {
  ...
  {
  ?>
  <option value="<?php echo htmlentities($rw['blockName']);?>"><?php echo htmlentities($rw['blockName']);?></option>
<?php
}}
?>
...
<div class="col-sm-4">
<input type="text" name="roomnumber" required="required" value="<?php echo htmlentities($row['roomnumber']);?>" class="form-control">
</div>
</div>
